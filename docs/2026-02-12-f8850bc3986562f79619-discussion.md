...
## Option 1 vs Option 2 (decision tension)

- Option 1: main risk is choosing a single minimum interval for software mode; a fixed value is policy and may not fit all hardware or scenarios.
- Option 2: avoids rearming into the past, but very small intervals can still cause high-frequency timer activity and may still starve RCU under load.

## Current stance

- The tradeoff is non-trivial, which likely contributes to the lack of upstream progress.
- We will pause this issue for now and revisit later; next step is to pick another issue to work on.
