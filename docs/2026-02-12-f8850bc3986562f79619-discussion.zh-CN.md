...
- RCU stall 往往被当作“资源饥饿症状”，而不是明确可隔离的 bug，维护者可能在等更清晰的修复方案。
- 若触发条件是软件模式下极小的 interval，可能被认为不现实，优先级降低。
- TAPRIO 处在 net/sched 与 TSN 交界，负责维护的人相对少，排查进度慢。
- syzbot 报告通常在有候选补丁并验证后才会真正推动。

## 关于“现实性”与评审定位

- 目前 repro 依赖非常小的 interval，这在软件模式的常见部署中并不现实。
- 但这仍可视为鲁棒性/DoS 风格输入校验问题，作为评审时的定位是合理的。

## 选项 1 与选项 2 的取舍

- 选项 1：主要难点是软件模式最小 interval 的取值，固定阈值带有政策性质，可能无法覆盖所有硬件/场景。
- 选项 2：避免“设到过去”的重启，但 interval 过小时仍会产生高频定时器活动，在压力下依然可能造成 RCU 饥饿。

## 当前结论

- 取舍较复杂，这也是上游迟迟没有进展的可能原因之一。
- 暂时搁置该问题，之后再回到本 issue；下一步先选择另一个问题继续。
