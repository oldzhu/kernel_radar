...
[简体中文](2026-02-04-f8850bc3986562f79619-taprio-rcu-stall-rca.zh-CN.md)
...
### Pros/cons & regression risk (Options A/B/C)

These options are **not mutually exclusive**. A common strategy is to apply A+B as a fast, low-risk mitigation, then consider C as a more “correct” long-term behavior fix.

#### Option A (minimum interval in software mode)

Pros:
- Strong, simple defense: blocks pathological schedules at configuration time; prevents the timer storm entirely.
- Very small change surface: mostly schedule validation + netlink extack message.
- Predictable performance: avoids “high rate timer” designs that general-purpose CPUs cannot honor reliably.

Cons:
- Behavior change: schedules that were previously accepted will now be rejected.
- Choosing the threshold is policy: `1us` is conservative; some users may argue for smaller.

Potential regressions / behavior impact:
- If someone intentionally uses **sub-microsecond** gate intervals in software mode, their configuration will fail.
- For normal TAPRIO deployments (intervals in tens/hundreds of microseconds or milliseconds), there should be no effect.
- Does not directly address “late execution” semantics; it just prevents extreme inputs.

#### Option B (soft hrtimer mode)

Pros:
- Reduces worst-case system impact: moves execution out of hard-IRQ context so it’s less likely to monopolize CPU in interrupt context.
- Low semantic risk for typical workloads: still runs the same callback; only changes *where* it runs.

Cons:
- Less precise timing: soft timers can be delayed more than hard hrtimers under load.
- Not a complete fix alone: a very small interval can still generate very frequent callbacks; it’s just less catastrophic than hard-IRQ starvation.

Potential regressions / behavior impact:
- TAPRIO gate transitions in pure software mode may become **slightly more jittery** under CPU pressure.
- If an application expects extremely precise gate timings without HW offload, this may worsen timing accuracy (but that expectation is already unrealistic on loaded systems).

#### Option C (catch-up / skip-forward)

Pros:
- Most “semantically correct” under overload: avoids re-arming the timer into the past and converges to the entry corresponding to `now`.
- Prevents busy-loop behavior even with moderately small intervals.
- Improves robustness: if the system is late (VM pause, CPU contention, long IRQ-off sections), it can recover gracefully.

Cons:
- Highest complexity: needs careful time-domain handling (base_time, cycle boundaries, admin schedule switch, extension, etc.).
- Harder to prove correct: risk of subtle bugs around cycle restart, `cycle_time_extension`, and admin schedule base_time switching.

Potential regressions / behavior impact:
- Under overload, the qdisc will **skip missed gate transitions** (by design). This is generally preferable to stalling the CPU, but it is a behavior change.
- A buggy implementation could cause incorrect gate selection, schedule switch timing, or unexpected packet release patterns.

#### Interaction notes

- A + B together: safest mitigation set; blocks extreme intervals and reduces hard-IRQ impact for borderline cases.
- C complements A/B: even with a minimum interval, `advance_sched()` can still be invoked late; catch-up logic makes lateness safe.
